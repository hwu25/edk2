  ...
  switch (BusTiming) {
    ...
    case SdMmcMmcHsSdr:
      if ((((ExtCsd->DeviceType & BIT1)  != 0) && (Capabilities->HighSpeed != 0))) {
        Supported = TRUE;
      } else if ((((ExtCsd->DeviceType & BIT0)  != 0) && (Capabilities->HighSpeed != 0))) {
        Supported = TRUE;
      }
      break;
    case SdMmcMmcLegacy:
  ...
  )
{
  ...
  if (PreferedBusWidth != EDKII_SD_MMC_BUS_WIDTH_IGNORE &&
      IsBusWidthSupported (Private, SlotIndex, BusTiming, PreferedBusWidth)) {
  ...
  )
{
  ...
  if (PreferedDriverStrength.Emmc != EDKII_SD_MMC_DRIVER_STRENGTH_IGNORE &&
